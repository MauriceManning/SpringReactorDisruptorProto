	...
	<div class="page-module">
		...
		<div id="alt_navigation" class="page-module-body">
			...
			<ul id="reports-and-stats-list">
                ...
                <c:if test="${not devEdition}">
                    <spring:url value="/config/analyzers" var="url" />
                    <c:choose>
                        <c:when test="${not empty module and module eq 'analyzers'}">
                            <li class="active"><a href="${fn:escapeXml(url)}">Analyzers</a></li>
                        </c:when>
                        <c:otherwise>
                            <li><a href="${fn:escapeXml(url)}">Analyzers</a></li>
                        </c:otherwise>
                    </c:choose>
                </c:if>
			</ul>
			<insight:admin development="false">
				...
				<ul id="support-tools-list">
                    ...
                    <spring:url value="/config/agent-download" var="url" />
                    <c:choose>
                        <c:when test="${not empty module and module eq 'agent-download'}">
                            <li class="active"><a href="${fn:escapeXml(url)}">Download Agent</a></li>
                        </c:when>
                        <c:otherwise>
                            <li><a href="${fn:escapeXml(url)}">Download Agent</a></li>
                        </c:otherwise>
                    </c:choose>
				</ul>
			</insight:admin>
...
<c:choose>
    ...
    <c:when test="${module eq 'analyzers'}">
        <script type="text/javascript">
            dojo.addOnLoad(function() {
                dojo.require("insight.components.SortableTable");
                new insight.components.SortableTable({
                    url: Insight.buildUri("/services/config/analyzers"),
                    defaultSortSettings: {
                        field: "name",
                        desc: false
                    }
    ...
    <c:when test="${module eq 'agent-download'}">
        <script type="text/javascript">
            dojo.addOnLoad(function() {
                dojo.require("insight.components.PageModule");
                new insight.components.PageModule({
                    url: Insight.buildUri("/services/config/agent-download", {'d': '1'})
                }, "config-body");
            });
        </script>
    </c:when>
    <c:when test="${module eq 'threshold'}">
		<script type="text/javascript">
			dojo.addOnLoad(function() {
...
<script type="text/javascript">
	<insight:renderPluginResources area="CONFIG_BODY_SCRIPT"/>
</script>
