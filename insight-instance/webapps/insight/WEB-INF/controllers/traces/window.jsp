			...
			<thead>
				<tr>
					...
					<c:if test="${not devEdition}">
						<th class="sort-field sort-field-serverLabel">Instance</th>
					</c:if>
			...
			<c:forEach items="${traceSummaries}" var="traceSummary" end="200" varStatus="status">
				<tbody id="trace_list_${fn:escapeXml(traceSummary.id)}" class="trace_list_item">
					<tr>
                        ...
                        <c:choose>
 							<c:when test="${traceSummary.errorCount eq 0}">
                                <td class="numeric" style="vertical-align: top;" rowspan="2">
                            </c:when>
                            <c:otherwise>
                                <td class="numeric hasErrors" style="vertical-align: top;" rowspan="2" alt="This trace has errors" title="This trace has errors">
                            </c:otherwise>
                        </c:choose>
	                        <a href="javascript:Insight.loadTrace(${fn:escapeXml(traceSummary.id)}, true);">${fn:escapeXml(duration)} ms</a>
	                    </td>
