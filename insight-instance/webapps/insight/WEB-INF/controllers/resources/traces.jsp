					...
					<tr id="trace_list_${fn:escapeXml(traceSummary.id)}" class="trace_list_item" trace-date="<fmt:formatDate value="${traceSummary.range.startDate}" pattern="yyyy-MM-dd'T'HH:mm:ssZ" />">
						<fmt:formatNumber value="${traceSummary.range.durationMillis}" var="duration" />
                 		<c:choose>
                            ...
                            <c:otherwise>
                                <td align="right" class="hasErrors" style="background-position: 5px 0px;" alt="This trace has errors" title="This trace has errors">
                            </c:otherwise>
                        </c:choose>
                                <a href="javascript:Insight.loadTrace(${insight:escapeJs(traceSummary.id)}, true);">${fn:escapeXml(duration)} ms</a></td>
