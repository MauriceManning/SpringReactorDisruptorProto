...
<c:if test="${not resource.resource.type.responseTimeAware}">
	<div class="submodule">
		<h2 style="float: left;">Throughput Trend</h2>
		<div id="throughputTrend" style="height: 150px; clear: both"></div>
		<script type="text/javascript">
			dojo.addOnLoad(function() {
				...
				var chart = new insight.charting.MultiResourceChart({
					timeRange: timeRange,
					dataPoints: 30,
					url: Insight.buildUri("/services/resources/{resource}/{start}/{end}/throughput", {
						resource: resource.toString(),
						start: timeRange.getStart(),
						end: timeRange.getEnd()
					}),
					yAxisArgs: { type: insight.charting.axis2d.Metric, vertical: true, includeZero: true, fixed: false, minorTicks: false, labelFunc: "_throughputLabelFunc" }
				}, "throughputTrend");
