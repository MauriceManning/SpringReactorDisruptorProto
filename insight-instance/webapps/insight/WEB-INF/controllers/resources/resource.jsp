...
<c:choose>
	<c:when test="${invalidResource}">
		...
		<script type="text/javascript">
			Insight.logEvent("Resource", "View Invalid");
		</script>
		<div class="page-module">
			...
			<div class="page-module-body">
				...
				<c:if test="${not devEdition}">
					It's likely this resources just doesn't exist on this server instance.
					Try selecting 'All Instances' in the tree.
				</c:if>
			</div>
		</div>
	</c:when>
	...
	<c:when test="${not empty resource and not resource.resource.type.responseTimeAware}">
	...
	<c:when test="${not empty resource and resource.resource.type.responseTimeAware}">
		...
		dojo.addOnLoad(function(){
			...
			<c:if test="${resource.resource.type.relatedToApplication}">
				menu.addChild(new dijit.MenuItem({
					...
					onClick: function(){
						Insight.logEvent("MenuItem", "Click", "Related to Application");
			...
			<c:if test="${resource.resource.type.relatedToServer}">
				menu.addChild(new dijit.MenuItem({
					label: "Instance ${insight:escapeJs(resource.resource.server)}",
					onClick: function(){
						Insight.logEvent("MenuItem", "Click", "Related to Instance");
			...
			<c:if test="${resource.resource.type.relatedToApplicationServer}">
				menu.addChild(new dijit.MenuItem({
					label: "Application ${insight:escapeJs(resource.resource.applicationName.formatted)} on Instance ${insight:escapeJs(resource.resource.server)}",
					onClick: function(){
						Insight.logEvent("MenuItem", "Click", "Related to ApplicationInstance");
